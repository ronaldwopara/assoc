#!/usr/bin/env node
/**
 * Optimizes bhm/*.jpg to WebP and uploads to Cloudinary
 * folder: asosc/gallery-prev/black-history-month
 * public_ids: bhm25-1..n (avoid overwriting existing bhm-2025-* assets used for 2026)
 */
...
const SOURCE_DIR = path.join(ROOT, "bhm");
const OUT_DIR = path.join(ROOT, "bhm", ".optimized");
const FOLDER = "asosc/gallery-prev/black-history-month";
const QUALITY = 75;
...
async function main() {
  ...
  const outJson = path.join(ROOT, "scripts/.bhm-upload-results.json");
