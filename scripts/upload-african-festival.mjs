#!/usr/bin/env node
/**
 * Optimizes af/*.jpg to WebP and uploads to Cloudinary
 * folder: asosc/gallery-prev/african-festival
 */
...
const SOURCE_DIR = path.join(ROOT, "af");
const OUT_DIR = path.join(ROOT, "af", ".optimized");
const FOLDER = "asosc/gallery-prev/african-festival";
const QUALITY = 75;
...
function publicIdFromFile(filename) {
  // AfricanFestival-1.jpg -> afc-2025-1
  const match = filename.match(/AfricanFestival-(\d+)/i);
  const num = match?.[1] ?? path.parse(filename).name;
  return `afc-2025-${num}`;
}
...
async function main() {
  ...
  if (sources.length === 0) throw new Error(`No JPEGs found in ${SOURCE_DIR}`);
  ...
  const outJson = path.join(ROOT, "scripts/.african-festival-upload-results.json");
